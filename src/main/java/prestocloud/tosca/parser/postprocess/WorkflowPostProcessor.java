...
import prestocloud.paas.wf.validation.AbstractWorkflowError;
import prestocloud.paas.wf.validation.InlinedWorkflowNotFoundError;
import prestocloud.paas.wf.validation.UnknownNodeError;
import prestocloud.paas.wf.validation.UnknownRelationshipError;
import prestocloud.paas.wf.validation.WorkflowHasCycleError;
    ...
    private void finalizeParsedWorkflows(TopologyContext topologyContext, Node node) {
        ...
        for (Workflow wf : topologyContext.getTopology().getWorkflows().values()) {
            ...
            try {
                ...
                log.trace(
                        "Not found exceptions during fill host id occurs when a relationship specified in workflow does not exist. This exceptions is ignored as the workflow validation trigger errors for such situations.",
                        e);
            }
